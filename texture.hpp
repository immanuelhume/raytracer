...
class Noisy : public Texture
{
  public:
    ...
    Noisy(const color &c, double scale = 1.0);
  ...
  public:
    ...
    Perlin noise_;
    color c_ = color(1, 1, 1, 1); // black by default
};
...
class Turbulence : public Noisy
{
  public:
    ...
    Turbulence(const color &c, double scale = 1.0);
    virtual color ValueAt(double u, double v, const point &p) const override;
};
...
class Marble : public Noisy
{
  public:
    ...
    Marble(Axis ax, const color &c, double scale = 1.0);
    virtual color ValueAt(double u, double v, const point &p) const override;
...
class Wood : public Noisy
{
  public:
    ...
    Wood(const color &c, double scale = 1.0, double f = 8.0);
    virtual color ValueAt(double u, double v, const point &p) const override;
